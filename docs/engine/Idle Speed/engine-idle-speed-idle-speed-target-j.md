# Engine - Idle Speed - Idle Speed Target J
...
| **Dimensions** | 0x16 |
| **Data Unit** | RPM |
| **Source File** | `Engine - Idle Speed - Idle Speed Target J - 2018 - LF9C102P.csv` |

## Description

Defines target idle RPM based on coolant temperature for operating condition J. This is the last in a series of idle speed target tables (A through J) used by the ECU to determine desired idle speed under various operating conditions. The ECU selects among these tables based on combinations of A/C compressor state, transmission position, electrical load status, and other parameters.

Table J typically represents a high-load combination scenario, possibly with multiple accessories active. Each table provides a temperature-based idle RPM curve, with higher RPM at cold temperatures and lower RPM when fully warmed.
...
1. **Condition Check**: ECU determines if Table J conditions are active
...
- **Engine - Idle Speed - Target A-I**: Other idle target tables
